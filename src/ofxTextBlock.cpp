      ...
      for (int l = 0; l < lines.size(); l++) {
         ...
         for (int w = 0; w < lines[l].wordsID.size(); w++) {
            ...
            if (words[currentWordID].rawWord == " ")
               ...
               nonSpaceWordWidth += words[currentWordID].width;
         }
   ...
   if (words.size() > 0) {
      ...
      for (int l = 0; l < lines.size(); l++) {
         ...
         if (lines[l].wordsID.size() > 0) {
            if (words[lines[l].wordsID[0]].rawWord == " ")
               lines[l].wordsID.erase(lines[l].wordsID.begin());
         }

         //Delete the last word if it is a blank
         if (lines[l].wordsID.size() > 0) {
            if (words[lines[l].wordsID[lines[l].wordsID.size() - 1]].rawWord == " ")
               lines[l].wordsID.erase(lines[l].wordsID.end() - 1);
         }
      }
   }
}
